...
from tools.platform_compat import _IS_WINDOWS, get_detached_popen_kwargs, shell_join
...
class GatewayRunner:
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        ...
        hermes_cmd_str = shell_join(hermes_cmd)
        update_cmd = (
        ...
        try:
            shell_bin = _find_shell()
            setsid_bin = None if _IS_WINDOWS else shutil.which("setsid")
            popen_kwargs = {
            ...
            if setsid_bin:
                ...
                subprocess.Popen(
                    [setsid_bin, shell_bin, "-c", update_cmd],
                    **popen_kwargs,
                )
            else:
                # Fallback: detached child process using the platform shell.
                subprocess.Popen(
                    [shell_bin, "-c", update_cmd],
                    **popen_kwargs,
                )
        except Exception as e:
